function proc_video(videoDir, outDir, category, step, startFrame, endFrame)
...
for k = startFrame : step : endFrame
	...
	tic
	load(category);
	[dets, boxes, info, pyraInfo, score] = imgdetect(im, model, model.thresh);
	toc
	% show the image
	outName = fullfile(outDir, ['image', num2str(k), '.mat']);
	save(outName, 'dets', 'boxes', 'pyraInfo', 'score');
end
